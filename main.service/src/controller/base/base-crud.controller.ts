...
export class BaseCrudController<MODEL> implements IBaseCrudController<MODEL> {
  ...
  async searchBase(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const searchData: SearchDataDto = getSearchData(req);

      const result = await (this.service as any).searchBase(searchData);

      res.send_ok('Searched successfully', result);
    } catch (error) {
      next(error);
    }
  }
